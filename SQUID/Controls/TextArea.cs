    ...
    public class TextArea : Control, IText
    {
            ...
            foreach (TextLine line in Lines)
            {
                for (int i = 0; i < line.Elements.Count; i++)
                {
                    ...
                    if (rect.Contains(m))
                    {
                        ...
                        Point mb = m - p;
                        int font = UI.Renderer.GetFont(line.Elements[i].Font);
                        int off = 0;
                        int c = 0;
        ...
        protected override void DrawText(Style style, float opacity,int charsToDraw)
        {
            ...
            if (Lines.Count == 0)
            {
                ...
                {
                    p1 = Location;
                    font = UI.Renderer.GetFont(style.Font);
            ...
            foreach (TextLine line in Lines)
            {
                ...
                foreach (TextElement element in line.Elements)
                {
                    ...
                    if (element.IsLink && element.Href == ActiveHref)
                        UI.Renderer.DrawBox(p2.x, p2.y, size.x - 1, size.y, ColorInt.FromArgb(opacity, LinkColor));
