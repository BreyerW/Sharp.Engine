...
namespace Fossil
{
	public static class Delta
	{
		...
		public static byte[] Create(ReadOnlySpan<byte> origin, ReadOnlySpan<byte> target)
		{
		...
		private static uint Checksum(ReadOnlySpan<byte> arr, uint bufferLength = 0)
		{
			uint sum0 = 0, sum1 = 0, sum2 = 0, sum = 0,
			 N = bufferLength is 0 ? (uint)arr.Length : bufferLength;
			int z = 0;
			while (N >= 16)
			{
