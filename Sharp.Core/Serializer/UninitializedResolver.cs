		...
		protected override IList<JsonProperty> CreateProperties(Type type, MemberSerialization memberSerialization)
		{
			...
			while (hierarchy.TryPop(out var t))
			{
				foreach (var f in t.GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance)
						.Where(x => x is FieldInfo fi && ((fi.GetCustomAttribute<JsonIgnoreAttribute>(true) is null && fi.IsPrivate is false) || (fi.IsPrivate && x.GetCustomAttribute<JsonIncludeAttribute>(true) is not null))))
					members.TryAdd(f.Name, f);

				foreach (var p in t.GetProperties(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance)
							.Where(x =>
							x is PropertyInfo pi && pi.Name.StartsWith("get_")/*pi.GetIndexParameters().Length is 0*/ && ((pi.GetCustomAttribute<JsonIgnoreAttribute>(true) is null && pi.GetSetMethod(true) is { } mi && mi.IsPublic)
