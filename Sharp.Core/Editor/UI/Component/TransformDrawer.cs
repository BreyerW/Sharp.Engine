...
namespace Sharp.Core.Editor.UI.Component
{
	...
	class TransformDrawer : ComponentDrawer<Transform>//TODO: remove oninitializedgui by making componentdrawer control itself?
	{
		...
		{
			position = new Vector3Field(() => (Target as Transform).Position, (pos) => (Target as Transform).Position = pos);
			...
			scale = new Vector3Field(() => (Target as Transform).Scale, (pos) => (Target as Transform).Scale = pos);
