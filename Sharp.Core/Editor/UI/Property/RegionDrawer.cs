...
namespace Sharp.Editor.UI.Property
{
	...
	static partial class Registerer
	{
		...
		{
			InspectorView.RegisterDrawerFor<RangeAttribute>(() => new RegionDrawer());
		}
	}
	public class RegionDrawer : PropertyDrawer<Curve[]>//or IList<Curve>?
	{
		...
		static RegionDrawer()
		{
			...
			polyfill2dMat.BindProperty(Material.MESHSLOT, MeshPipeline.GetAsset("dynamic_polyfill"));

			line2dMat = new Material();
			line2dMat.BindShader(0, (Shader)ShaderPipeline.Import(Application.projectPath + @"\Content\LineShader.shader"));
			line2dMat.BindProperty(Material.MESHSLOT, MeshPipeline.GetAsset("dynamic_curve"));
		}
		...
		{
			var V = Value as Curve[];
			List<Vector2> list = new List<Vector2>();
			if (V is null)
			{
				return list.ToArray();
			}
			list.Capacity = 1000 + V.Length;
		...
		private void AddPoints(ref List<Vector2> points, float minTime, float maxTime, float visibleMinTime, float visibleMaxTime, int curveId)
		{
			...
			var keys = V[curveId].keys;
			if (keys[0].time > minTime)
			{
				points.Add(new Vector2(curvesRange.x, V[curveId].Evaluate(curvesRange.x)));
			...
			foreach (var i in ..(keys.Length - 1))
			{
				...
				if (keyframe2.time >= minTime && keyframe.time <= maxTime)
				{
					...
					points.Add(new Vector2(num, V[curveId].Evaluate(num)));
		...
		protected override void DrawAfter()
		{
			...
			ref var polyfill = ref MeshPipeline.GetAsset("dynamic_polyfill");
			...
			ref var mesh = ref MeshPipeline.GetAsset("dynamic_curve");
