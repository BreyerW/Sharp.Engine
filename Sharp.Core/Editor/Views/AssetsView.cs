	...
	public class AssetsView : View//TODO: nested entity, complete gizmo
	{
		...
		public AssetsView(uint attachToWindow) : base(attachToWindow)
		{
			...
			if (directories.Count == 0)
				ConstructFlatDirectoryTree(new DirectoryInfo(Application.projectPath + @"\Content"));
			BuildAssetViewTree();
		...
		{
			var dirs = Directory.EnumerateDirectories(Application.projectPath + @"\Content", "*", SearchOption.AllDirectories);
		...
		private void BuildAssetViewTree()//rebuild on focusgained
		{
			...
			foreach (var (key, files) in directories)
			{
				...
				foreach (var folder in key.Split(new string[] { Application.projectPath, "\\" }, StringSplitOptions.RemoveEmptyEntries))
				{
					if (folder != @"Content")
					{
