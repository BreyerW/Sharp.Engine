...
namespace SharpSL.BackendRenderers
{
    public class UIRenderer : ISquidRenderer
    {
        ...
        static UIRenderer()
        {
            var shader = ShaderPipeline.Import(Application.projectPath + @"\Content\SDFShader.shader");
            sdfMaterial = new Material();
            sdfMaterial.BindShader(0, shader);
            sdfMaterial.BindProperty("color", fontColor);
            sdfMaterial.BindProperty(Material.MESHSLOT, MeshPipeline.GetAsset("square"));

            squareMaterial = new Material();
            squareMaterial.BindShader(0, ShaderPipeline.Import(Application.projectPath + @"\Content\ColoredEditorShader.shader"));
            squareMaterial.BindProperty(Material.MESHSLOT, MeshPipeline.GetAsset("square"));
            squareMaterial.BindProperty("len", Vector2.One);

            var square = MeshPipeline.GetAsset("dynamic_square");

            texturedSquareMaterial = new Material();
            texturedSquareMaterial.BindShader(0, ShaderPipeline.Import(Application.projectPath + @"\Content\TexturedEditorShader.shader"));
            texturedSquareMaterial.BindProperty(Material.MESHSLOT, square);
        }
        ...
        public void DrawText(string text, int x, int y, int width, int height, int font, int color, float fontSize)//TODO: split this to draw texture and draw mesh
        {
            ...
            ref var realFont = ref FontPipeline.GetAsset(font);
            var col = new Color((uint)color);
        ...
        {
            ref var texture2d = ref TexturePipeline.GetAsset(texture);
            var col = new Color((uint)color);

            var mat = Matrix4x4.CreateScale(width, height, 1) * Matrix4x4.CreateTranslation(x, y, 0) * MainEditorView.currentMainView.camera.OrthoMatrix;

            ref var mesh = ref MeshPipeline.GetAsset("dynamic_square");
        ...
        {
            ref var f = ref FontPipeline.GetAsset(font);
            var v = Vector2.Zero;
            float xoffset = 0;
            float yoffset = 0;
            float newHeight = 0;
        ...
        {
            TexturePipeline.Import(Application.projectPath + @"\SharpSL\BackendRenderers\Content\" + name);
