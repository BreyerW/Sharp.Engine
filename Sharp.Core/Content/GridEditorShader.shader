...
float log10(float x) { return  (1.0f / log(10.0f)) * log(x); }
...
void main(void)
{
	...
	const float min_pixels_between_cells = 3.0f;
	...
	vec4 thick_color2 = mix(thick_color, vec4(1, 0.25f, 0.25f, 1), float(abs(uv.x / dudv.x) < 1.0f && abs(uv.y / dudv.y) > 1.0f && abs(uv.y / dudv.y) < len / abs(dudv.y)));
	thick_color2 = mix(thick_color2, vec4(0.26f, 0.6f, 1, 1), float(abs(uv.y / dudv.y) < 1.0f && abs(uv.x / dudv.x) > 1.0f && abs(uv.x / dudv.x) < len / abs(dudv.x)));
	...
	float op_distance = 1.0f - saturate(distance(cam_pos.xz, uv) / (abs(cam_pos.y) * camNearFar.y * 0.005f + camNearFar.y * 0.25f)); //1000f zfar of camera
	...
	c.a *= mix(mix((lod0_a * (1.0f - lod_fade)), lod1_a, float(lod1_a > 0)), lod2_a, float(lod2_a > 0));// *op;
