    ...
    public class VerticalSplitter : Base
    {
		...
		protected void OnHorizontalMoved(Base control, EventArgs args)
        {
			...
			if (OnSplitMoved != null)
				OnSplitMoved (m_HSplitter, EventArgs.Empty);
        }

        private float CalculateValueHorizontal()
        {
			if (m_HSplitter.X < MinimumSize.X)
				m_HSplitter.X = MinimumSize.X;
			else if (m_HSplitter.X > Width - MinimumSize.X)
				m_HSplitter.X = Width - MinimumSize.X;
