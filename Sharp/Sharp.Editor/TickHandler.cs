...
namespace Sharp.Editor
{
    internal class TickHandler
    {
        ...
        {
            if (frameRate != (float)Math.Round((double)frameRate))
            {
            ...
            else
            {
                ...
                while ((float)num < frameRate)
                {
                    ...
                    int num2 = (int)Math.Round((double)(frameRate / (float)num));
                    if (num2 % 60 == 0)
                    {
                        num *= 2;
                        list.Add(num);
                    ...
                    {
                        num = (int)Math.Round((double)frameRate);
                    }
                }
        ...
        public float[] GetTicksAtLevel(int level, bool excludeTicksFromHigherlevels)
        {
            ...
            {
                if (!excludeTicksFromHigherlevels || num >= this.m_BiggestTick || i % (int)Math.Round((double)(this.m_TickModulos[num + 1] / this.m_TickModulos[num])) != 0)
                {
