...
namespace SharpSL.BackendRenderers
{
	public class UIRenderer : ISquidRenderer
	{
		...
		public void DrawText(string text, int x, int y, int width, int height, int font, int color, float fontSize)//TODO: split this to draw texture and draw mesh
		{
			...
			ref var realFont = ref Pipeline.Get<FontPipeline>().GetAsset(font);
		...
		{
			ref var texture2d = ref Pipeline.Get<TexturePipeline>().GetAsset(texture);
			var col = new Color((uint)color);
		...
		{
			ref var f = ref Pipeline.Get<FontPipeline>().GetAsset(font);
		...
		{
			Pipeline.Get<TexturePipeline>().Import(@"B:\Sharp.Engine3\Sharp\SharpSL\BackendRenderers\Content\" + name);
