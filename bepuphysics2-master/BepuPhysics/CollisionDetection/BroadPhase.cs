...
namespace BepuPhysics.CollisionDetection
{
	...
	public unsafe partial class BroadPhase : IDisposable
	{
		...
		public Buffer<CollidableReference> FrozenLeaves;
		/// <summary>
		...
		{
			FrozenTree.GetBoundsPointers(index, out minPointer, out maxPointer);
		}
		/// <summary>
		...
		{
			FrozenTree.UpdateBounds(broadPhaseIndex, min, max);
		}
		...
		void ExecuteRefitAndMark(int workerIndex)
		{
			...
			while (true)
			{
				...
				else if (jobIndex >= activeRefineContext.RefitNodes.Count && jobIndex < activeRefineContext.RefitNodes.Count + staticRefineContext.RefitNodes.Count)
				{
					jobIndex -= activeRefineContext.RefitNodes.Count - frozenRefineContext.RefitNodes.Count;
				...
				{
					jobIndex -= activeRefineContext.RefitNodes.Count - staticRefineContext.RefitNodes.Count;
		...
		{
			var threadPool = threadDispatcher.GetThreadMemoryPool(workerIndex);
			var maximumSubtrees = Math.Max(Math.Max(activeRefineContext.MaximumSubtrees, staticRefineContext.MaximumSubtrees), frozenRefineContext.MaximumSubtrees);
			...
			while (true)
			{
				...
				else if (jobIndex >= activeRefineContext.RefinementTargets.Count && jobIndex < activeRefineContext.RefinementTargets.Count + staticRefineContext.RefinementTargets.Count)
				{
					jobIndex -= activeRefineContext.RefinementTargets.Count - frozenRefineContext.RefinementTargets.Count;
					Debug.Assert(jobIndex >= 0 && jobIndex < staticRefineContext.RefinementTargets.Count);
					staticRefineContext.ExecuteRefineJob(ref subtreeReferences, ref treeletInternalNodes, ref resources, threadPool, jobIndex);
				}
				else
				{
					jobIndex -= activeRefineContext.RefinementTargets.Count - staticRefineContext.RefinementTargets.Count;
		...
		public void Update(IThreadDispatcher threadDispatcher = null)
		{
			...
			if (threadDispatcher != null)
			{
				...
				remainingJobCount = activeRefineContext.RefitNodes.Count + staticRefineContext.RefitNodes.Count + frozenRefineContext.RefitNodes.Count;
				...
				remainingJobCount = activeRefineContext.RefinementTargets.Count + staticRefineContext.RefinementTargets.Count + frozenRefineContext.RefinementTargets.Count;
